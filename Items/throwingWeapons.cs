using MechThrowverhaul.Projectiles;
...
namespace MechThrowverhaul.Items
{
    public class ThrowingWeapons : GlobalItem
    {
        ...
        public override void UpdateInventory(Item item, Player player)
        {
            ...
            {
                if (item.stack > 1) // this is for thrown weapons that spawn naturally with stacks higher than 0
                {
        ...
        {
            bool consume = true;
            //prevents using the weapon when there is 0 ammo left. If the player has a chance not to consume thrown weapons, apply it here. Consumes 1 ammo from the weapon otherwise
            if (throwverhauled == true && throwStack == 0)
            {
                return false;
            }
        ...
        // also shows the text and makes the sound of picking up an item
